...
int scs_sqR_Lasso_solve(double * X, double * Y, double lambda, int n, int p, double * beta){
	...
	for(j=0; j<p; j++){
		...
		vn[j] = F77_NAME(ddot)(&n, X+(j*n), &one, X+(j*n), &one);
		vn[j] = sqrt(n/vn[j]);
	}
	...
	//for(i=0; i<n; i++) (d->b)[i] = Y[i]; 
	F77_NAME(dcopy)(&n, Y, &one, d->b, &one);
	...
	/*for (j = 0; j < p; j++){
		beta[j] = (double) (sol->x)[p+j] * vn[j];
	}*/
	F77_NAME(dsbmv)("L", &p, &zero, &done, (sol->x)+p, &one, vn, &one, &dzero, beta, &one); 
